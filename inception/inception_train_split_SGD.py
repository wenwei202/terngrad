...
def _tower_loss(images, labels, num_classes, scope, reuse_variables=None):
  ...
  for l in losses + [total_loss] + [total_regularization_loss]:
...
def train(dataset):
  ...
  with tf.Graph().as_default(), tf.device('/cpu:0'):
      ...
      with tf.device('/gpu:%d' % i):
        with tf.name_scope('%s_%d' % (inception.TOWER_NAME, i)) as scope:
          with tf.variable_scope('%s_%d' % (inception.TOWER_NAME, i)):
            ...
            tower_entropy_grads.append(grads)
    ...
    if 1 == FLAGS.grad_bits:
      for grads in tower_entropy_grads:
      ...
      for i in xrange(FLAGS.num_gpus):
        with tf.device('/gpu:%d' % i):
          with tf.name_scope('%s_%d' % (inception.TOWER_NAME, i)) as scope:
            ...
            tower_entropy_grads[i] = bingrad_common.clip_gradients_by_thresholds(
              tower_entropy_grads[i],mean_scalers)
      ...
      for i in xrange(FLAGS.num_gpus):
        with tf.device('/gpu:%d' % i):
          with tf.name_scope('%s_%d' % (inception.TOWER_NAME, i)) as scope:
            # Clip and binarize gradients
            # and keep track of the gradients across all towers.
            tower_entropy_grads[i] = bingrad_common.stochastical_binarize_gradients(
              tower_entropy_grads[i], mean_scalers)
    ...
    # synchronization point across all towers @ CPU.
    if len(tower_entropy_grads)>1:
      tower_entropy_grads = bingrad_common.average_gradients2(tower_entropy_grads)
    ...
    for i in xrange(FLAGS.num_gpus):
      with tf.device('/gpu:%d' % i):
        with tf.name_scope('%s_%d' % (inception.TOWER_NAME, i)) as scope:
          ...
          apply_gradient_op.append(opt.apply_gradients(tower_entropy_grads[i],
